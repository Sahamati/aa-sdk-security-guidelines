...
Note: Optional
If needs to be used then whitelist FIU's url in your x-frame-options and Content Security Policy header.
...
SDK should control the UI that is rendered but provide configurability on themeing and alignment of the widget.
...
Note: Optional, if AA login is PIN vs an OTP.
...
Note: Mandatory
It is important to validate that all the fields that are configured while initialising the SDK.
And also validation of the input fields (vua, pin, otp etc,.)
...
3. Point 13 needs to considered here as well.
...
### 11. Consent access should be restricted to the FIU that created it.
Note: Mandatory
 - The SDK or the AA should not allow approval, rejection and revocation of the consent created by anothe FIU.

### 12. Protection of discovered information such as Account discovery list.
 - Potential solution: 
   - Server side render the bank list and serve it in an iframe.
   - Render a list of images (like captcha) and insert into dom.

### 13. Authenticate each FIU to trace usage and pinpoint leaks.
 - Method of auth is left to the AA
 - TODO: list best practices for client side authentication
    - Token based auth via TOTP

### 14. CSRF headers could be used as way to control access to only whitelisted FIUs or parties.
Note: Optional.
- Prevention of CORS
